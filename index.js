...
app.get('/', (req, res) => {
    res.send('Bot is Running with Reply Keyboard Submission!');
});
app.listen(port, () => {
...
bot.on('message', async (msg) => {
    ...
    if (text === '❌ Cancel') {
        userState[chatId] = null;
        if (isAdmin && userState[chatId] !== 'WAITING_FILE') {
             // যদি এডমিন প্যানেলের কোনো কাজে থাকে
            bot.sendMessage(chatId, "❌ Cancelled.", { reply_markup: getAdminKeyboard(chatId, config) });
        } else {
            // ইউজার বা ফাইল সাবমিশন ক্যান্সেল করলে
            bot.sendMessage(chatId, "❌ Cancelled.", { reply_markup: getMainMenu(chatId, isAdmin) });
        }
        return;
    }

    // --- ফাইল সাবমিশন (বাটন চেঞ্জ) ---
    if (text === '📂 Submit File') {
        if (user.banned) {
            bot.sendMessage(chatId, "🚫 <b>ACCESS DENIED</b>\nYou are banned from submitting files. Contact support.", { parse_mode: 'HTML' });
            return;
        }
        // এখানে Reply Keyboard শো করা হচ্ছে
    ...
    if (userState[chatId] && userState[chatId].state === 'WAITING_FILE') {
        ...
        // ডবল চেক (আপলোড করার আগ মুহূর্তে যদি এডমিন বন্ধ করে দেয়)
    ...
    if (text === 'ℹ️ Use Info') {
        bot.sendMessage(chatId, useInfoText.bn, { parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: "🔄 Translate English", callback_data: "lang_en" }]] } });
        return;
    }
    if (text === '📞 Support') {
        bot.sendMessage(chatId, "📞 <b>24/7 CUSTOMER SUPPORT</b>\n➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖\nNeed help? Contact our admin directly.\n➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖", { parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: "💬 Contact Admin", url: config.supportLink || "https://t.me/YourUsername" }]] } });
        return;
    }
    ...
    if (isAdmin) {
        ...
        if (text === '🔄 Reset Date') {
            ...
            bot.sendMessage(chatId, "🔒 <b>Security Check</b>\nEnter password:", { parse_mode: 'HTML', reply_markup: cancelKeyboard });
            return;
        }
        if (userState[chatId] === 'RESET_DATE_PASS') {
            if (text === 'MTS@2026') {
                await updateConfig({ lastDate: "" }); 
                bot.sendMessage(chatId, "✅ <b>Success!</b> Date reset.", { parse_mode: 'HTML', reply_markup: getAdminKeyboard(chatId, config) });
            } else {
                bot.sendMessage(chatId, "❌ <b>Wrong Password!</b>", { parse_mode: 'HTML', reply_markup: getAdminKeyboard(chatId, config) });
            }
            userState[chatId] = null;
            return;
        }
                ...
                for (const id of allIds) {
                    if (id != chatId) {
                        ...
                        bot.sendMessage(id, "⚠️ <b>System Update!</b>\n\nআসসালামু আলাইকুম সবাই নতুন ফিচার ব্যবহার করার জন্য Start / update দিন", {
                            parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: "🔄 Update Now / Restart", callback_data: "restart_bot" }]] }
                        }).catch(()=>{});
                    }
                }
                await batch.commit(); 
                bot.sendMessage(chatId, "✅ <b>Done.</b>", {parse_mode:'HTML', reply_markup: getAdminKeyboard(chatId, config)});
            } else {
                bot.sendMessage(chatId, "❌ Cancelled.", {reply_markup: getAdminKeyboard(chatId, config)});
            }
            userState[chatId] = null;
        ...
        if (chatId == mainAdminId) {
            ...
            if (userState[chatId] === 'ADD_ADMIN') {
                ...
                else { bot.sendMessage(chatId, "⚠️ Already Admin.", {reply_markup: getAdminKeyboard(chatId, config)}); }
            ...
            if (userState[chatId] === 'REM_ADMIN') {
                ...
                else { bot.sendMessage(chatId, "⚠️ Not an Admin.", {reply_markup: getAdminKeyboard(chatId, config)}); }
                userState[chatId] = null; return;
            }
        }
...
console.log("🚀 Bot Running with Original Welcome & Reply Keyboard...");
