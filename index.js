...
app.get('/', (req, res) => {
    res.send('Bot is Running with Advanced Categories & Support!');
});
app.listen(port, () => {
    ...
    try {
        ...
        if (!doc.exists) {
            data = {
                ...
                // New Fields for Categories & Support
                instaCategories: [], // Array of { name, active, closedMsg }
                fbCategories: [],    // Array of { name, active, closedMsg }
                supportButtons: []   // Array of { name, link }
            };
        ...
        if (!data.supportButtons) data.supportButtons = []; // For multiple support admins
...
// জেনেরিক ফাংশন ক্যাটাগরি কিবোর্ড এর জন্য
function getCategoryKeyboard(categories, backButtonText) {
    ...
    keyboard.push([{ text: backButtonText }]); // Back button
...
function getAdminKeyboard(userId, config) {
    ...
    // মেইন এডমিন এর জন্য এক্সট্রা
    if (userId == mainAdminId) {
        kb.splice(5, 0, [{ text: "👥 Manage Support Admins" }]); // Support admin control
...
bot.on('callback_query', async (query) => {
    ...
    try {
        ...
        // ক্যাটাগরি টগল (এডমিন)
            ...
            // format: cat_toggle_TYPE_INDEX (e.g., cat_toggle_IG_0)
            const parts = data.split('_');
            const type = parts[2]; // IG or FB
...
bot.on('message', async (msg) => {
    ...
    // Back button inside Category Menu
    ...
    if (userState[chatId] && userState[chatId].state === 'SELECTING_CAT') {
        const mainType = userState[chatId].mainType; // INSTAGRAM or FACEBOOK
    ...
    if (userState[chatId] && userState[chatId].state === 'WAITING_FILE') {
        ...
        if (msg.document && msg.document.file_name && (msg.document.file_name.endsWith('.xlsx') || msg.document.file_name.endsWith('.xls'))) {
            ...
            // Payment selection in REPLY KEYBOARD as requested
            let kbRows = [];
            let tempRow = [];
            methods.forEach(m => {
                tempRow.push({ text: m });
    ...
    // --- PAYMENT SELECTION & NUMBER INPUT ---
    // If waiting for payment select (Button Click)
    ...
    // If waiting for Number Input (Final Step)
    ...
    if (text === '📞 Support') {
        ...
        buttons.forEach(btn => {
            inlineKB.push([{ text: `💬 ${btn.name}`, url: btn.link }]);
        });

        // Add Main Support link if set
        if (config.supportLink) {
            inlineKB.push([{ text: "💬 Main Admin Support", url: config.supportLink }]);
        }

        if (inlineKB.length === 0) {
            inlineKB.push([{ text: "💬 Contact Admin", url: "https://t.me/YourUsername" }]);
        }
    ...
    if (text === '🎥 Bot Use Video') {
        if (config.tutorialVideoId) {
            bot.sendVideo(chatId, config.tutorialVideoId, { caption: "🎥 <b>How to use</b>", parse_mode: 'HTML' });
        } else {
            bot.sendMessage(chatId, "⚠️ No tutorial video set.", { parse_mode: 'HTML' });
        }
        return;
    }
    if (text === '👤 Profile') {
    ...
    if (isAdmin) {
        ...
        if (text === '📂 Manage Insta Cat') {
            ...
            if (cats.length === 0) {
                ...
                bot.sendMessage(chatId, "📂 <b>Instagram Categories:</b>\nClick to Toggle ON/OFF or Delete.", {
                    parse_mode: 'HTML',
                    reply_markup: { inline_keyboard: buttons }
                });
                // Show Add button in bottom keyboard
        ...
        if (text === '➕ Add Insta Category') {
            userState[chatId] = 'ADD_INSTA_CAT_NAME';
            bot.sendMessage(chatId, "📝 Enter Category Name:", { reply_markup: cancelKeyboard });
            return;
        }
        if (userState[chatId] === 'ADD_INSTA_CAT_NAME') {
            let cats = config.instaCategories || [];
            // Add new category (Default Active)
        ...
        if (text === '📂 Manage FB Cat') {
            ...
            if (cats.length === 0) {
                ...
                bot.sendMessage(chatId, "📂 <b>Facebook Categories:</b>\nClick to Toggle ON/OFF or Delete.", {
                    parse_mode: 'HTML',
                    reply_markup: { inline_keyboard: buttons }
                });
                bot.sendMessage(chatId, "Select action:", {
        ...
        if (text === '➕ Add FB Category') {
            userState[chatId] = 'ADD_FB_CAT_NAME';
            bot.sendMessage(chatId, "📝 Enter Category Name:", { reply_markup: cancelKeyboard });
            return;
        }
        ...
        // --- HANDLE CATEGORY DISABLE MESSAGE INPUT ---
        ...
        // --- SUPPORT ADMINS (Main Admin Only) ---
        if (text === '👥 Manage Support Admins') {
            ...
            if (buttons.length > 0) {
                ...
                bot.sendMessage(chatId, "📋 <b>Current Support Admins:</b>", {
                    parse_mode: 'HTML',
            ...
            bot.sendMessage(chatId, "👇 Options:", {
                reply_markup: {
                    keyboard: [[{ text: "➕ Add Support Admin" }], [{ text: "🔙 Back to Admin" }]], resize_keyboard: true
                }
            });
            return;
        }

        if (text === '➕ Add Support Admin') {
            userState[chatId] = 'ADD_SUPP_NAME';
            bot.sendMessage(chatId, "📝 Enter Button Name (e.g., Admin Rahim):", { reply_markup: cancelKeyboard });
            return;
        }
        if (userState[chatId] === 'ADD_SUPP_NAME') {
            userState[chatId] = { state: 'ADD_SUPP_LINK', name: text };
            bot.sendMessage(chatId, "🔗 Enter Telegram Link/Username (e.g., @username or https://t.me/...):", { reply_markup: cancelKeyboard });
            return;
        }
        if (userState[chatId] && userState[chatId].state === 'ADD_SUPP_LINK') {
            let link = text;
            if (link.startsWith('@')) link = `https://t.me/${link.substring(1)}`;
            else if (!link.startsWith('http')) link = `https://t.me/${link}`;

            let buttons = config.supportButtons || [];
            buttons.push({ name: userState[chatId].name, link: link });
            await updateConfig({ supportButtons: buttons });

            bot.sendMessage(chatId, `✅ Added Support: <b>${userState[chatId].name}</b>`, { parse_mode: 'HTML', reply_markup: getAdminKeyboard(chatId, config) });
            userState[chatId] = null;
            return;
        }
        ...
        if (text === '💳 Manage Payment') {
            const methods = config.paymentMethods || [];
            bot.sendMessage(chatId, `💳 <b>Current Methods:</b>\n${methods.join(', ')}`, {
                parse_mode: 'HTML',
                reply_markup: {
                    keyboard: [
        ...
        if (text === '➕ Add Payment Method') {
            ...
            bot.sendMessage(chatId, "📝 Enter Payment Method Name:", {reply_markup: cancelKeyboard});
            return;
        }
        ...
        if (text === '🗑 Delete Payment Method') {
            const methods = config.paymentMethods || [];
            if (methods.length === 0) {
                bot.sendMessage(chatId, "⚠️ Empty.", {reply_markup: getAdminKeyboard(chatId, config)});
                return;
            }
        ...
        // --- GLOBAL CONTROL (Previous logic retained) ---
        ...
        // ... (Global ON/OFF logic same as previous code, simplified here)
        if (text === '🟢 Global Insta: ON') { await updateConfig({ instaActive: false }); bot.sendMessage(chatId, "🔴 Insta Closed Globally."); return; }
        if (text === '🔴 Global Insta: OFF') { await updateConfig({ instaActive: true }); bot.sendMessage(chatId, "🟢 Insta Opened Globally."); return; }
        if (text === '🟢 Global FB: ON') { await updateConfig({ fbActive: false }); bot.sendMessage(chatId, "🔴 FB Closed Globally."); return; }
        if (text === '🔴 Global FB: OFF') { await updateConfig({ fbActive: true }); bot.sendMessage(chatId, "🟢 FB Opened Globally."); return; }

        // --- OTHER ADMIN FEATURES ---
        if (text === '🎥 Manage Video') {
             // ... (Video logic same as previous)
             bot.sendMessage(chatId, "🎥 <b>Video Settings</b>", {
                reply_markup: { keyboard: [[{text: "📤 Set New Video"}], [{text: "🗑 Remove Video"}], [{text: "🔙 Back to Admin"}]], resize_keyboard: true }
            });
            return;
        }
        ...
        if (userState[chatId] === 'WAITING_VIDEO_FILE' && msg.video) {
            await updateConfig({ tutorialVideoId: msg.video.file_id });
            bot.sendMessage(chatId, "✅ Saved.", {reply_markup: getAdminKeyboard(chatId, config)});
            userState[chatId] = null;
            return;
        }
        ...
        if (text === '🆔 Manage Channels') {
            bot.sendMessage(chatId, "🆔 <b>Manage Channels</b>", {
                reply_markup: {
                    keyboard: [
                        [{text: "🌐 Set Global"}, {text: "📸 Set Insta"}, {text: "🔵 Set FB"}],
                        [{text: "🔙 Back to Admin"}]
                    ], resize_keyboard: true
                }
            });
        ...
        // Add/Remove Admin logic (Same as before)
        ...
        if (userState[chatId] === 'ADD_ADMIN') {
            config.admins.push(parseInt(text)); await updateConfig({admins: config.admins});
        ...
        if (userState[chatId] === 'BROADCAST') {
            const all = await getAllUsersID();
            all.forEach(id => bot.sendMessage(id, `📢 <b>NOTICE</b>\n${text}`, {parse_mode:'HTML'}).catch(()=>{}));
        ...
        if (userState[chatId] === 'REP_1') { userState[chatId] = {step:'REP_2', t:text}; bot.sendMessage(chatId, "Message:"); return; }
        if (userState[chatId]?.step === 'REP_2') {
            bot.sendMessage(userState[chatId].t, `📨 <b>Admin Message:</b>\n${text}`, {parse_mode:'HTML'}).catch(()=>{});
            bot.sendMessage(chatId, "✅ Sent.", {reply_markup: getAdminKeyboard(chatId, config)}); userState[chatId]=null; return;
        }
    }
});
console.log("🚀 Updated Bot Running...");
