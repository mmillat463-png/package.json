...
app.get('/', (req, res) => res.send('Bot & MongoDB Debug Mode On!'));
app.listen(port, () => console.log(`🌍 Server running on port ${port}`));

// --- BOT SETUP (সিম্পল পোলিং) ---
// সরাসরি polling: true ব্যবহার করছি, এটি সবচেয়ে স্টেবল
...
async function getUser(id, name) {
    // এখানে লগ চেক করা হবে
    console.log(`👤 Database: Finding user ${id}...`);
    let user = await User.findOne({ userId: id });
    if (!user) {
        console.log(`👤 Database: Creating new user ${id}...`);
        user = new User({ userId: id, name: name });
        await user.save();
    }
    console.log(`✅ Database: User ${id} ready.`);
    return user;
}
...
bot.on('message', async (msg) => {
    ...
    // ডিবাগ লগ: মেসেজ ঢুকছে কিনা দেখার জন্য
