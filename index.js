...
async function getConfig(forceUpdate = false) {
    ...
    // তাহলে ডাটাবেসে হিট না করে মেমোরি থেকে ডাটা দেবে।
    ...
    try {
        ...
        // মেমোরিতে সেভ করা (পরবর্তী ব্যবহারের জন্য)
        cachedConfig = data;
        lastConfigFetch = now;
        ...
        // এরর হলে ক্যাশ থেকে দেওয়ার চেষ্টা করবে, তাও না থাকলে ডিফল্ট
...
async function updateConfig(data) {
    try {
        // ১. আগে ডাটাবেসে পার্মানেন্টলি সেভ করা
        await settingsColl.doc('main_config').set(data, { merge: true });
        
        // ২. সাথে সাথে মেমোরি আপডেট করা (যাতে ইউজাররা ইনস্ট্যান্ট রেজাল্ট পায়)
        if (cachedConfig) {
            cachedConfig = { ...cachedConfig, ...data };
        } else {
            // যদি ক্যাশ খালি থাকে, নতুন করে আনা
...
bot.on('message', async (msg) => {
    ...
    // ১. অপ্টিমাইজড কনফিগ আনা (DB খরচ কমবে)
    const config = await getConfig();
